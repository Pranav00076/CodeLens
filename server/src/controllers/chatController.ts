...
export class ChatController {
  static async askQuestion(req: Request, res: Response): Promise<void> {
    ...
    let keyFiles: Array<{ path: string; sampleContent?: string; language?: string; lines: number }> = [];
    ...
    const domainMeta = {
      ...
      manifestInfo: {
        projectType: repo.architecture.pattern.includes('Chrome') ? 'chrome_extension' as const : 'general_app' as const,
        keywords: [],
        scripts: {},
