...
export class AnalyzeController {
   ...
   */
  static async getDemo(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  static async analyzeGithub(req: Request, res: Response): Promise<void> {
    const { url } = req.body;
    if (!url) {
    ...
    try {
      ...
      aiResult.issues.forEach(issue => {
        if (issue.severity === 'high') healthScore -= 8;
        else if (issue.severity === 'medium') healthScore -= 4;
        else healthScore -= 2;
      });
   ...
   */
  static async analyzeZip(req: Request, res: Response): Promise<void> {
    const file = req.file;
    if (!file) {
  ...
  private static prioritizeFiles(
    allFiles: Array<{ relativePath: string; absolutePath: string; lines: number; sampleContent?: string; language?: string }>,
    entryPoints: Array<{ path: string }>,
    domainMeta: any
  ): Array<{ path: string; lines: number; language?: string; sampleContent?: string }> {
    const selected = new Map<string, { path: string; lines: number; language?: string; sampleContent?: string }>();
    ...
    const manifests = allFiles.filter(f =>
      /(?:package\.json|manifest\.json|pyproject\.toml|Cargo\.toml|go\.mod|docker-compose\.ya?ml|schema\.prisma)$/i.test(f.relativePath)
    );
    ...
    entryPoints.forEach(ep => {
      ...
      if (match) selected.set(match.relativePath, { path: match.relativePath, lines: match.lines, language: match.language, sampleContent: match.sampleContent });
    });
