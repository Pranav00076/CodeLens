...
import { ChatMessage, Citation, GeneratedTest } from '../types/index.js';
...
export class GeminiProvider implements IAIProvider {
  ...
  async analyzeCodebase(context: RepoAIContext): Promise<AIAnalysisResult> {
    ...
    try {
      ...
      if (!rawText) return this.fallback.analyzeCodebase(context);
  ...
  async generateTests(context: RepoAIContext, targetFile: string, fileContent: string, targetFunction?: string, framework: string = 'Vitest'): Promise<GeneratedTest> {
    ...
    try {
      ...
      const data = (await response.json()) as any;
      const rawText = data?.candidates?.[0]?.content?.parts?.[0]?.text;
      if (!rawText) return this.fallback.generateTests(context, targetFile, fileContent, targetFunction, framework);
