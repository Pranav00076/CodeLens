...
export interface ProjectManifestInfo {
  name?: string;
  description?: string;
  projectType: 'chrome_extension' | 'fullstack_web' | 'cli_tool' | 'mobile_app' | 'backend_api' | 'library' | 'ai_service' | 'general_app';
  keywords: string[];
...
export interface DomainMetadata {
  ...
  readmeProblemStatement?: string;
  readmeFeatures: string[];
  readmePrerequisites: string[];
  readmeCommands: Array<{ label: string; command: string; description: string }>;
  manifestInfo: ProjectManifestInfo;
  identifiedModules: Array<{ name: string; path: string; purpose: string }>;
}
...
export class DomainParserService {
   ...
   */
  static extractDomainMetadata(
    rootDir: string,
    scannedFiles: Array<{ relativePath: string; absolutePath: string; sampleContent?: string; lines: number; language?: string }>
  ): DomainMetadata {
    // 1. Locate and parse README
    const readmeFile = scannedFiles.find(f =>
      /(?:^|\/)(readme|read_me|readme\.md|readme\.markdown|readme\.txt|readme\.rst)$/i.test(f.relativePath)
    );
  ...
  private static parseReadme(content: string): {
    title: string;
    tagline: string;
    overview: string;
    ...
    const commands: Array<{ label: string; command: string; description: string }> = [];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        (currentSection.includes('feature') || currentSection.includes('what it does') || currentSection.includes('highlights') || currentSection.includes('capabilities')) &&
      ...
      if (
        (currentSection.includes('overview') || currentSection.includes('about') || currentSection.includes('introduction') || currentSection.includes('what is') || currentSection.includes('mission')) &&
        !overview &&
        line.length > 20 &&
        !line.startsWith('#') &&
        !line.startsWith('```') &&
      ...
      // Problem statement
      if (
        (currentSection.includes('problem') || currentSection.includes('why') || currentSection.includes('motivation') || currentSection.includes('goal')) &&
        !problemStatement &&
        line.length > 20 &&
        !line.startsWith('#') &&
        !line.startsWith('<')
      ...
      if (currentSection.includes('prerequisite') && (line.startsWith('- ') || line.startsWith('* '))) {
        ...
        if (prereq.length > 3) prerequisites.push(prereq);
      }

      // Commands in code blocks under Installation or Getting Started
      if (
        (currentSection.includes('install') || currentSection.includes('getting started') || currentSection.includes('setup') || currentSection.includes('run') || currentSection.includes('develop')) &&
        line.startsWith('```')
      ) {
  ...
  private static parseManifests(
    rootDir: string,
    scannedFiles: Array<{ relativePath: string; absolutePath: string }>
  ): ProjectManifestInfo {
    const manifestInfo: ProjectManifestInfo = {
    ...
    if (pyproject && fs.existsSync(pyproject.absolutePath)) {
      try {
        ...
        if (descMatch) manifestInfo.description = descMatch[1];
        manifestInfo.projectType = 'backend_api';
      } catch {
        // ignore
      }
  ...
  private static identifyKeyModules(
    scannedFiles: Array<{ relativePath: string; lines: number; language?: string }>
  ): Array<{ name: string; path: string; purpose: string }> {
