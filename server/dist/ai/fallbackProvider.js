...
export class FallbackProvider {
    ...
    async analyzeCodebase(context) {
        ...
        if (isChromeExt)
            pattern = 'Chrome Manifest V3 Extension & On-Device Pipeline';
        else if (isMicroservices)
            pattern = 'Microservices & Distributed Services';
        else if (isNext)
            pattern = 'Server-Side Rendered Fullstack (Next.js)';
        else if (isReact)
            pattern = 'Client-Server Decoupled Single-Page App';
        else if (isDjango || isFastAPI)
            pattern = 'Model-View-Controller / Clean API';
        ...
        const issues = this.detectCodeIssues(context);
        ...
        // 7. Execution Flow
        const executionFlow = isChromeExt
            ? [
                {
                    step: 1,
